...
namespace ServerPart.EF.ConfigFluentAPI
{
    public class StudentCourseConfig : IEntityTypeConfiguration<StudentCourse>
    {
        public void Configure(EntityTypeBuilder<StudentCourse> entity)
        {
            ...
            entity.HasOne(p => p.Student).WithMany(p => p.StudentCourse).IsRequired().OnDelete(DeleteBehavior.Restrict);
            entity.HasOne(p => p.Course).WithMany(p => p.StudentCourse).IsRequired().OnDelete(DeleteBehavior.Restrict);
        }
    }
